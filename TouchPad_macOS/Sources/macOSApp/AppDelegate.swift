...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    // 【修改】删除了这里的局部变量，改用 AppState.shared.isLocked
    ...
    func rebuildMenu() {
        ...
        let stylusItem = NSMenuItem(title: "仅接收触控笔 (防误触)", action: #selector(changeModeStylus), keyEquivalent: "")
        stylusItem.target = self
        stylusItem.state = (AppState.shared.inputMode == .stylusOnly) ? .on : .off
        
        let bothItem = NSMenuItem(title: "触控笔 + 手指", action: #selector(changeModeBoth), keyEquivalent: "")
    ...
    @objc func toggleLock() {
        ...
        if AppState.shared.isLocked {
            ...
            NSLog("[TouchPad] 发送锁定指令 (状态已同步至 Global)")
        } else {
            sendCommandToClient("CMD_UNLOCK")
            NSLog("[TouchPad] 发送解锁指令 (状态已同步至 Global)")
        }
        rebuildMenu()
    }
    ...
    @objc func selectDevice(_ sender: NSMenuItem) {
        if let deviceId = sender.representedObject as? String {
            ...
            rebuildMenu()
            NSLog("[TouchPad] 切换控制设备: \(deviceId)")
        }
    }
