...
class ScreenStreamer {
    ...
    func start() {
        guard !isStreaming else {
            NSLog("[Streamer] Already running")
            return
        }
        isStreaming = true
        ...
        guard videoServerSocket != -1 else {
            NSLog("[Streamer] ERROR: cannot create socket")
            isStreaming = false
            return
        }
        ...
        guard bindRes != -1 else {
            NSLog("[Streamer] ERROR: bind failed")
            close(videoServerSocket)
            isStreaming = false
            return
        }
        ...
        streamLoop = Thread { [weak self] in
            while self?.isStreaming == true {
                ...
                if sock > 0 {
                    ...
                    while self?.isStreaming == true && recv(sock, &buffer, 1, 0) > 0 {
                        sleep(1)
                    }
                    ...
                    // 【关键修复】连接断开后，重置 isStreaming 状态
                    // 这样当数据线重新插上时，start() 方法不会被 "Already running" 阻塞
    ...
    func stop() {
        isStreaming = false
        if let sock = clientSocket {
            close(sock)
            clientSocket = nil
        }
        if videoServerSocket != -1 {
            close(videoServerSocket)
            videoServerSocket = -1
        }
        streamLoop?.cancel()
    ...
    private func setupCompressionSession() -> Bool {
        ...
        guard let screen = NSScreen.main else {
            NSLog("[Streamer] ERROR: cannot get main screen")
            return false
        }
        ...
        guard status == noErr, let s = session else {
            NSLog("[Streamer] ERROR: VTCompressionSessionCreate failed \(status)")
            return false
        }
        ...
        VTSessionSetProperty(s, key: kVTCompressionPropertyKey_DataRateLimits, value: NSNumber(value: 0))
        ...
        let handler: CGDisplayStreamFrameAvailableHandler = { (status, displayTime, frameSurface, updateRef) in
            ...
            if result == kCVReturnSuccess, let pb = pixelBuffer?.takeRetainedValue() {
                self.encodeQueue.async {
                    self.encodePixelBuffer(pb, presentationTime: displayTime)
                }
            }
        }
    ...
    private func encodePixelBuffer(_ pixelBuffer: CVPixelBuffer, presentationTime: UInt64) {
        ...
        if status != noErr {
            NSLog("[Streamer] Encode frame failed: \(status)")
        }
    }
    ...
    private func handleEncodedSample(sampleBuffer: CMSampleBuffer) {
        guard let sock = clientSocket else {
            return
        }
        ...
        if frameCount == 0 {
            if let (spsData, ppsData) = getParameterSets(from: sampleBuffer) {
                func sendPacket(rawPayload: Data) {
                    ...
                    header[0] = 1
                    for i in 0..<4 {
                        header[i+1] = UInt8(truncatingIfNeeded: len >> (8 * (3-i)))
                    }
